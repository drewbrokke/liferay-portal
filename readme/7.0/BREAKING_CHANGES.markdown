...
The USERS_LAST_NAME_REQUIRED property has been removed from the portal.properties and the corresponding UI.  Required names are now handled on a per-language basis via the language.properties files.
...
If you need to require the last name, simply list it on the `lang.user.name.required.field.names` line of the appropriate language.properties files like so: `lang.user.name.required.field.names=last-name`.
