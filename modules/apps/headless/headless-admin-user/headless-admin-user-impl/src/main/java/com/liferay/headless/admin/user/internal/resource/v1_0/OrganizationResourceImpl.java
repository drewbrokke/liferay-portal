...
public class OrganizationResourceImpl
	...
	public Organization postOrganization(Organization organization)
		...
		long parentOrganizationId = _getDefaultParentOrganizationId(
			organization);
		...
		long regionId = _getRegionId(organization, countryId);

		long statusId = ListTypeConstants.ORGANIZATION_STATUS_DEFAULT;
		String comments = organization.getComment();
		boolean site = false;
		List<Address> addresses = _getAddresses(organization);
		List<com.liferay.portal.kernel.model.EmailAddress> emailAddresses =
			_getEmailAddresses(organization);
		List<OrgLabor> orgLabors = _getOrgLabors(organization);
		List<com.liferay.portal.kernel.model.Phone> phones = _getPhones(
			organization);
		List<Website> websites = _getWebsites(organization);
