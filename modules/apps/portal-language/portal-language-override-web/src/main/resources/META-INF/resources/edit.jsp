...
<c:if test="<%= Validator.isNotNull(editDisplayContext.getKey()) %>">
	<aui:script sandbox="<%= true %>">
		const clearAllOverridesButton = document.getElementById(
			'<portlet:namespace />clearOverridesButton'
		);

		clearAllOverridesButton.addEventListener('click', (event) => {
			if (
				confirm(
					'<liferay-ui:message key="are-you-sure-you-want-to-reset-all-translation-overrides" />'
				)
			) {
				submitForm(
					document.getElementById(
						'<portlet:namespace />editPortalLanguageOverrideFm'
					),
