		...
		const registerOnSubmit = auiForm => {
			auiForm.set('onSubmit', (event) => {
				...
				for (const input of inputs) {
					if (input.name.startsWith('<portlet:namespace />value') && !!input.value) {
						return;
					}
				}
				...
				const keyField = inputs.find(input => input.name === '<portlet:namespace />key');

				keyField.focus();

				Liferay.Util.openToast({
					message: '<%= LanguageUtil.get(request, "at-least-one-translation-is-required-with-a-new-language-key") %>',
					type: 'danger'
				});
			});
		}
