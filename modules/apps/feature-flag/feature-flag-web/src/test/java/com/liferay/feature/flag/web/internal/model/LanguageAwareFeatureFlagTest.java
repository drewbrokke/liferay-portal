...
public class LanguageAwareFeatureFlagTest {
	...
	public void testGetOtherValues() {
		...
		Assert.assertEquals(
			_featureFlag.getStatus(), _languageAwareFeatureFlag.getStatus());
		Assert.assertEquals(
	...
	public void testGetTitle() {
		...
		Mockito.doReturn(
			...
			Mockito.any(Locale.class),
			Mockito.eq(
				FeatureFlagConstants.getKey(_featureFlag.getKey(), "title")),
			Mockito.any(String.class)
		);

		Assert.assertEquals(
			description, _languageAwareFeatureFlag.getTitle(_locale));
	}
