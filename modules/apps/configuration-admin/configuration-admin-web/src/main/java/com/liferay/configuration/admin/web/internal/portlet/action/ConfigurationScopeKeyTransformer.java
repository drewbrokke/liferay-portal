 ...
 * @author Drew Brokke
 */
@Component(service = ConfigurationScopeKeyTransformer.class)
public class ConfigurationScopeKeyTransformer {
	...
	public boolean prepareForExport(
		...
		for (ExtendedObjectClassDefinition.Scope scope :
			...
			// todo this is useless, and is actually a problem because the other key is already removed
			...
			if (_log.isInfoEnabled()) {
				_log.info(
					String.format(
						"For pid %s: replacing internal identifier %s with " +
							"portable identifier %s",
						pid, internalIdentifier, portableIdentifier));
			}

			properties.put(scope.getPortablePropertyKey(), portableIdentifier);
	...
	public boolean prepareForImport(
		...
		for (ExtendedObjectClassDefinition.Scope scope :
			...
			if (_log.isInfoEnabled()) {
				_log.info(
					String.format(
						"For pid %s: replacing portable identifier %s with " +
	...
	private static final Log _log = LogFactoryUtil.getLog(
		ConfigurationScopeKeyTransformer.class);
