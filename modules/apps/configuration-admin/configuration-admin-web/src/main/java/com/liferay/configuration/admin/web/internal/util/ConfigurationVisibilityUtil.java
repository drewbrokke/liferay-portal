...
public class ConfigurationVisibilityUtil {
	...
	public static boolean isVisible(
		...
		configurationVisibilityController = _serviceTrackerMap.getService(
			getVisibilityControllerKey(pid));
	...
	private static Class<?> getClass(Bundle bundle, String pid) {
		try {
	...
	private static Class<?> getClass(String pid) {
		Class<?> clazz = null;

		for (Bundle bundle : _bundleContext.getBundles()) {
			if (pid.startsWith(bundle.getSymbolicName())) {
				clazz = getClass(bundle, pid);
			}
	...
	private static String getVisibilityControllerKey(String pid) {
		Class<?> clazz = getClass(pid);
		...
		if (annotation == null) {
			if (_log.isDebugEnabled()) {
				_log.debug(
					"No ExtendedObjectClassDefinition annotation found on class " +
						pid);
			}
