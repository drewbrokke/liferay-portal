...
public class EditCompanyMVCActionCommand extends BaseFormMVCActionCommand {
	...
	private String _getRequiredLocaleMessageKey(List<Group> groups) {
		if (groups.isEmpty()) {
			...
			return "language-in-use-remove-assignment-to-x-x";
		}
		else if (groups.size() == 2) {
			return "language-in-use-remove-assignments-to-x-and-x-x";
		}

		return "language-in-use-remove-assignments-to-x-x-and-x-more-x";
	}
