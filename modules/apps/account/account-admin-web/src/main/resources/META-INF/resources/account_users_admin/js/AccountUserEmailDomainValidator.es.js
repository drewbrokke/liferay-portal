...
class AccountUserEmailDomainValidator extends PortletBase {
	...
	addFieldMessage_(field) {
		...
		button.onclick = () => {
			this.openDialog_(this.viewValidEmailDomainsURL);
		};
		button.type = 'button';
	...
	getEmailDomainFieldRule_() {
		const validEmailDomains = this.validEmailDomains.split(',');
...
AccountUserEmailDomainValidator.STATE = {
	validEmailDomains: Config.string,
	viewValidEmailDomainsURL: Config.string,
};
