...
public class AccountEntryLocalServiceTest {
	...
	public void testAccountEntryName() throws Exception {
		...
		try {
			_accountEntryLocalService.updateAccountEntry(
				...
				null, null, null, accountEntry.isActive(),
				ServiceContextTestUtil.getServiceContext());
	...
	public void testActivateAccountEntries() throws Exception {
		long[] accountEntryIds = _addAccountEntries(false);
	...
	public void testActivateAccountEntryByModel() throws Exception {
		...
		accountEntry = _accountEntryLocalService.activateAccountEntry(
			accountEntry);
	...
	public void testGetUserAccountEntriesByAccountEntryOwnership()
		...
		_testGetUserAccountEntries(
			accountEntryOwner,
			_addUserAccountEntry(
				accountEntryOwner.getUserId(), RandomTestUtil.randomString(),
				null, null));
	}
	...
	public void testSearchByActive() throws Exception {
		...
		_assertSearchWithParams(
			_getLinkedHashMap("active", true), activeAccountEntry);
		_assertSearchWithParams(null, activeAccountEntry);
		_assertSearchWithParams(
			_getLinkedHashMap("active", false), inactiveAccountEntry);
		_assertSearchWithParams(
			_getLinkedHashMap("active", null), activeAccountEntry,
	...
	private AccountEntry _addUserAccountEntry(
		...
		AccountEntry accountEntry = _accountEntryLocalService.addAccountEntry(
			...
			RandomTestUtil.randomString(), null, null, null, null, type, true,
			null);
	...
	private void _assertActive(long accountEntryId, boolean expectedActive) {
		...
		Assert.assertEquals(expectedActive, accountEntry.isActive());
	}
	...
	private void _assertSearchWithParams(
		...
		for (AccountEntry accountEntry :
			...
			ArrayUtil.contains(expectedAccountEntries, accountEntry);
		}
	}
	...
	private void _testGetUserAccountEntries(
		...
		for (int i = 0; i < expectedAccountEntries.length; i++) {
			...
			Assert.assertEquals(
				expectedAccountEntry.isActive(), userAccountEntry.isActive());
			Assert.assertEquals(
