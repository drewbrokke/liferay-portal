...
public class CETConfigurationFactory {
		...
		ConfigurationFactoryUtil.executeAsCompany(
			...
			companyId -> {
				...
				if (Objects.equals(
					...
					_addThemeCSSClientExtensionEntryRel(companyId);
				}
			});
	}
	...
	private void _addThemeCSSClientExtensionEntryRel(Long companyId)
		...
		if (clientExtensionEntryRelsCount > 1) {
			_log.error(
				"Only one theme CSS client extension can be applied to the " +
