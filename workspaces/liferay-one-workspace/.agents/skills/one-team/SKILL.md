...
- Every dispatch names the phase, the assignment, the artifact paths, and what done looks like — complete, in one message, inside the ten-line budget. Each extra round-trip re-processes that teammate's whole transcript, so one full dispatch costs less than three partial ones, and a probe only pays for itself when a circuit breaker calls for it.
