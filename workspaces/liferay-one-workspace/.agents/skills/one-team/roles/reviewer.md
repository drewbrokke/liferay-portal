...
**Without the flag — the default — you run the review yourself** through the skill's steps, fanning the lenses out at the size threshold Step 3 names. It costs a fraction of the adversarial shape and finds less; the coordinator decides which the ticket is worth, and `review.md` says which one ran so nobody has to guess later.
